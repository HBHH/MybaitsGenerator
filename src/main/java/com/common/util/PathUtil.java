...
public class PathUtil {
	...
	public static String getClassesRealPath() {
		...
		// /D:/java/tomcat5.5/webapps/aokle/WEB-INF/lib/aokle1.0.0.0.jar
	 ...
	 * @param request
	 * @param scheme
	 * @return http://localhost:8080/qyc
	 */
	...
	/**
	 * 返回当前访问页面的完整URL地址，即浏览器当前的完整地址，如：http://192.168.10.190:8080/aokle/index.jsp
	 * ?ab=dsds&sdds=12<br>
	 * request.getRequestURI()=/aokle/index.jsp <br>
	 * request.getRequestURL()=http://192.168.10.190:8080/aokle/index.jsp<br>
	...
	public static String[] getUrlPostArray(HttpServletRequest request) {
		Map<String, Object> parmsSet = request.getParameterMap();
		if(parmsSet == null){
			return null;
		}
		Object[] keyValuePairs = parmsSet.entrySet().toArray();
		String aryParams[] = null;
		if (parmsSet.size() > 0) {
			aryParams = new String[parmsSet.size()];
			for (int i = 0; i < parmsSet.size(); i++) {
				Map.Entry<String, Object> entry = (Map.Entry<String, Object>) keyValuePairs[i];
			...
			if (!(new File(filePath)).isFile()) {
				...
				if (!(new File(filePath)).isFile()) {
					...
					if (!(new File(filePath)).isFile()) {
						...
						filePath = PathUtil.getBaseRealPath() + "/src/main/webapp/WEB-INF/" + fileName;
						if (!(new File(filePath)).isFile()) {
